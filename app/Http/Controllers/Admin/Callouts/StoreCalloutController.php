...
final class StoreCalloutController extends Controller
{
    public function __invoke(Request $request): RedirectResponse
    {
        ...
        $callout = Callout::create($validated);

        if ( ! empty($members)) {
            $callout->members()->attach($members);
        }

        return redirect()
            ->route('admin.callouts.show', $callout)
            ->with('success', 'Callout created successfully.');
    }
}

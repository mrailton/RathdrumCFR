...
<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Callout</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update callout details</p>
        </div>
    </div>

    <!-- Form -->
    <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
        <form method="POST" action="{{ route('admin.callouts.update', $callout) }}" 
            x-data="{
                mobilised: {{ old('mobilised', $callout->mobilised ? '1' : '0') }},
                attended: {{ old('attended', $callout->attended ? '1' : '0') }},
                ohcaAtScene: {{ old('ohca_at_scene', $callout->ohca_at_scene ? '1' : '0') }},
                roscAchieved: {{ old('rosc_achieved', $callout->rosc_achieved ? '1' : '0') }}
            }"
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <x-admin.input 
                    ...
                    :value="old('incident_number', $callout->incident_number)"
                    :required="true" 
                />
                ...
                <x-admin.input 
                    ...
                    name="incident_date" 
                    type="datetime-local"
                    :value="old('incident_date', \Carbon\Carbon::parse($callout->incident_date)->format('Y-m-d\TH:i'))"
                    :required="true" 
                />
                ...
                <div class="sm:col-span-2">
                    <x-admin.select 
                        label="AMPDS Code" 
                        name="ampds_code" 
                        :value="old('ampds_code', $callout->ampds_code)"
                        :options="$ampdsCodes"
                        :required="true" 
                    />
                </div>
                ...
                <x-admin.select 
                    label="Gender" 
                    name="gender" 
                    :value="old('gender', $callout->gender)"
            ...
            <div class="space-y-6">
                ...
                <div x-show="mobilised == '0'" x-cloak>
                    ...
                    <div class="mt-2 space-y-2">
                        <label class="inline-flex items-center">
                            <input type="radio" name="medical_facility" value="0" {{ old('medical_facility', $callout->medical_facility) == '0' ? 'checked' : '' }} class="text-red-600 focus:ring-red-600">
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">No</span>
                        </label>
                        <label class="inline-flex items-center ml-6">
                            <input type="radio" name="medical_facility" value="1" {{ old('medical_facility', $callout->medical_facility) == '1' ? 'checked' : '' }} class="text-red-600 focus:ring-red-600">
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Yes</span>
                        </label>
                    </div>
                ...
                <div x-show="mobilised == '1'" x-cloak>
                    ...
                    >
                        @foreach($members as $id => $name)
                            <option value="{{ $id }}" {{ in_array($id, old('members', $callout->members->pluck('id')->toArray())) ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div x-show="attended == '1'" x-cloak class="space-y-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div x-show="ohcaAtScene == '1'" x-cloak class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <div class="mt-2 space-y-2">
                            <label class="inline-flex items-center">
                                <input type="radio" name="bystander_cpr" value="0" {{ old('bystander_cpr', $callout->bystander_cpr) == '0' ? 'checked' : '' }} class="text-red-600 focus:ring-red-600">
                                <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">No</span>
                            </label>
                            <label class="inline-flex items-center ml-6">
                                <input type="radio" name="bystander_cpr" value="1" {{ old('bystander_cpr', $callout->bystander_cpr) == '1' ? 'checked' : '' }} class="text-red-600 focus:ring-red-600">
                                <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Yes</span>
                            </label>
                        </div>
                        @error('bystander_cpr')
                    ...
                    <x-admin.select 
                        label="Source of AED" 
                        name="source_of_aed" 
                        :value="old('source_of_aed', $callout->source_of_aed)"
                    ...
                    <x-admin.input 
                        ...
                        type="number"
                        :value="old('number_of_shocks_given', $callout->number_of_shocks_given)"
                    />
                ...
                <div x-show="ohcaAtScene == '0' || roscAchieved == '1'" x-cloak>
                    ...
                    <div class="mt-2 space-y-2">
                        <label class="inline-flex items-center">
                            <input type="radio" name="patient_transported" value="0" {{ old('patient_transported', $callout->patient_transported) == '0' ? 'checked' : '' }} class="text-red-600 focus:ring-red-600">
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">No</span>
                        </label>
                        <label class="inline-flex items-center ml-6">
                            <input type="radio" name="patient_transported" value="1" {{ old('patient_transported', $callout->patient_transported) == '1' ? 'checked' : '' }} class="text-red-600 focus:ring-red-600">
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Yes</span>
                        </label>
                    </div>
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <x-admin.button variant="secondary" href="{{ route('admin.callouts.show', $callout) }}">
                    Cancel
                </x-admin.button>
            </div>
        </form>
